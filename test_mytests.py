                ...
                for l in range(len(m1)):
                    for n in range(len(RightWall)):
                        for o in range(len(tmax)):
                            result = m.CheckInputs(x1[i],3,0,0,x2[j],3,0,0,r1[k],0.5,m1[l],1,0,RightWall[n],0,10,tmax[o])
                            if z==0:
                                z += 1
...
def test_wallcollision():
    x, v = m.CollisionWall(x=1, v=1, r= 0.5, wall1=0, wall2=10) #check if nothing happens if there is no collision
    assert x == 1 and v == 1
    x, v = m.CollisionWall(x=0,v=-1,r=1, wall1=0, wall2=10) #check if collision happens correctly if there is collision
    assert x == 1 and v == 1
...
def test_animate():
    result = m.Animate([],[],[],[],0.1,0.5,0.5,0, 10, 0,10)
    assert result == "Error"
    result = m.Animate([3,4],[1,2],[1,2],[4,3,2],0.1,0.5,0.5,0, 10, 0,10)
    assert result == "Error"